...
app = typer.Typer(
    name="bactowise",
    help="Bacterial genome QC and annotation — one config file, one command.",
...
def validate(
    config: Path = typer.Option(
        ..., "-c", "--config",
        help="Pipeline config YAML to validate.",
        exists=True,
    ),
):
    """Validate a pipeline config file without running anything.
